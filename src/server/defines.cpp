...
pirates::ship::cabin_info::cabin_info(std::string_view t, std::string_view d)
...
zmq::multipart_t
pirates::ship::crew_header_mp(const server_id &s_id, const client_id &c_id,
...
zmq::message_t pirates::ship::make_zmq_msg(std::string_view str) {
  return zmq::message_t(str.data(), str.length());
}
